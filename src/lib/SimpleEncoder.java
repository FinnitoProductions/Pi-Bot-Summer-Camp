...
public class SimpleEncoder extends FeedbackSensor
{
    ...
    {        
        ...
        orangeInput.addListener(new GpioPinListenerDigital() {
            ...
            {
                if (changeEvent.getEdge().equals(PinEdge.RISING))
                {
                    addToPosition(brownInput.getState().equals(PinState.HIGH) ? -1 : 1);
                }
        ...
        brownInput.addListener(new GpioPinListenerDigital() {
            ...
            public void handleGpioPinDigitalStateChangeEvent(GpioPinDigitalStateChangeEvent changeEvent)
            {
                if (changeEvent.getEdge().equals(PinEdge.RISING))
                {
                    addToPosition(orangeInput.getState().equals(PinState.HIGH) ? 1 : -1);
                }
            }
        });
    }
